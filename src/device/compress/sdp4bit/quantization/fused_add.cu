...
#pragma unroll
            for (int k = 0; k < 8; k++) {
                ...
                data_cast[k] = (elem_offset + iteration * stride + k < elems_per_group) ? __hadd(data_cast[k], temp_param_model[k]) : __float2bfloat16(0.0f);
            }
            if(elem_offset + iteration * stride < elems_per_group){
...
/********* Launcher methods ***********/
#define LAUNCH_ADD_CALL()                            \
    fused_add_kernel<unroll_factor,                  \
                        internal_unroll_l,           \
                        threads_per_group,           \
                        max_threads>                 \
    ...
    cudaStream_t stream)
{
    ...
    for (size_t i = 0; i < num_params; ++i) {
        total_size += groups * elems_per_group / num_params;
        // param_list[i].size(0);
    ...
    if (is_subblock_schedule) {
        ...
        LAUNCH_ADD(1, internal_unroll, max_threads);
    } else if (external_unroll == 2) {
        LAUNCH_ADD(2, internal_unroll, max_threads);
    } else if (external_unroll == 3) {
        LAUNCH_ADD(3, internal_unroll, max_threads);
    } else if (external_unroll == 4) {
        LAUNCH_ADD(4, internal_unroll, max_threads);
    }
}
#endif
void launch_fused_add_cuda(
    ...
    // const int elems_per_group = at::numel(dequant_data) < 2048 ? at::numel(dequant_data) : 2048;
    // const int groups = at::numel(dequant_data) / elems_per_group;
    // const int elems_per_group = at::numel(input_vals) / groups;
    // if (dequant_data.scalar_type() == at::ScalarType::Half) {
    ...
    // } else if (dequant_data.scalar_type() == at::ScalarType::Float) {
        // launch_quant((int8_t*)output.data_ptr(),
        //             (float*)params.data_ptr(),
        ...
        // return {output, params};
    //     throw std::runtime_error("Unsupported input tensor data type.");
    // } else if (dequant_data.scalar_type() == at::ScalarType::BFloat16) {
        // launch_sub(
