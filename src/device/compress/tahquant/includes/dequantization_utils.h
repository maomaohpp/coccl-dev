...
template <typename T, int numBits, Type qType, int unroll, int threads>
DS_D_INLINE void _to_global_heuristic_ht(T* global_output,
                            const int8_t* data,
                            const float* global_params,
                            const int elems_per_group,
                            const int64_t total_elems,
...
#pragma unroll
    for (int i = 0; i < unroll; i++) {
        ...
        if (elem_id_iter < total_elems) {
            ...
            // const int group_index = elem_id_iter / elems_per_group;
            // Params<qType, numBits> q_params(global_params, group_index);
            const int64_t group_index = (elem_id_iter / elems_per_group);
...
template <typename T, int numBits, Type qType, int unroll, int threads>
DS_D_INLINE void to_global_heuristic_ht(T* global_output,
                           const int8_t* data,
                           const float* global_params,
                           const int elems_per_group,
                           const int64_t total_elems,
...
{
    if constexpr (numBits == 4 || numBits == 8) {
        _to_global_heuristic_ht<T, numBits, qType, unroll, threads>(
            global_output, data, global_params, elems_per_group, total_elems, flag);
    } else if constexpr (numBits == 3) {
        // TODO(cmikeh2): Need this implementation
        assert(false);
    } else {
