...
__global__ void dequantize_ht_kernel(T* __restrict__ dequant_data,
                                  ...
                                  const bool* flag)
{
    dequantize::to_global_heuristic_ht<T, numBits, qType, unroll, threads>(
        dequant_data, q_data, q_params, elems_per_group, total_elems, flag);
}

#define LAUNCH_DEQUANT_KERNEL_HEURISTIC_HT(num_bits, q_type)                                          \
    dequantize_ht_kernel<T, num_bits, q_type, unroll, threads><<<grid, block, 0, stream>>>( \
        dequant_data, q_data, q_params, elems_per_group, total_elems, flag);

template <typename T>
void launch_dequantize_heuristic_ht_kernel(T* dequant_data,
                              const int8_t* q_data,
                              ...
                              cudaStream_t stream)
{
    ...
    if (num_bits == 8 && q_type == quantize::Type::Symmetric) {
        LAUNCH_DEQUANT_KERNEL_HEURISTIC_HT(8, quantize::Type::Symmetric);
    } else if (num_bits == 8 && q_type == quantize::Type::Asymmetric) {
        LAUNCH_DEQUANT_KERNEL_HEURISTIC_HT(8, quantize::Type::Asymmetric);
    } else if (num_bits == 4 && q_type == quantize::Type::Symmetric) {
        LAUNCH_DEQUANT_KERNEL_HEURISTIC_HT(4, quantize::Type::Symmetric);
    } else if (num_bits == 4 && q_type == quantize::Type::Asymmetric) {
        LAUNCH_DEQUANT_KERNEL_HEURISTIC_HT(4, quantize::Type::Asymmetric);
    }
}

template void launch_dequantize_heuristic_ht_kernel(__half* dequant_data,
                                       const int8_t* q_data,
                                       const float* q_params,
                                       quantize::Type q_type,
                                       int num_bits,
...
template void launch_dequantize_heuristic_ht_kernel(float* dequant_data,
                                       const int8_t* q_data,
                                       const float* q_params,
                                       quantize::Type q_type,
                                       int num_bits,
...
template void launch_dequantize_heuristic_ht_kernel(__nv_bfloat16* dequant_data,
                                       const int8_t* q_data,
                                       const float* q_params,
                                       quantize::Type q_type,
                                       int num_bits,
