...
__device__ __forceinline__ void hadamard_mult_warp_quant(__nv_bfloat16 x[kNChunks * kNItems]) {
    ...
    // printf("lane_id %d\n", threadIdx.x );
    // printf("hadamard start! %lld \n", group_index);
    unsigned int MASK =  ((threadIdx.y * blockDim.x + threadIdx.x) % 32 )/ 16 == 0 ? 0x0000FFFF : 0xFFFF0000;

    // printf("kLogWarpSize %d kStepStart %d kNChunks %d kNItems %d\n", kLogWarpSize, kStepStart, kNChunks, kNItems);
    #pragma unroll
    for (int step = kStepStart; step < kLogWarpSize; ++step) {
        const int lane_mask = 1 << step;
        const float sign = (lane_id & lane_mask) ? -1.f : 1.f;
        ...
        for (int c = 0; c < kNChunks; ++c) {
            #pragma unroll
            for (int i = 0; i < kNItems; ++i) {
                // printf("__shfl_xor_sync start !\n");
                // float x_val_other = __bfloat162float(__shfl_xor_sync(FULL_MASK, x[c * kNItems + i], lane_mask));
                float x_val_other = __bfloat162float(__shfl_xor_sync(MASK, x[c * kNItems + i], lane_mask));
                // printf("__shfl_xor_sync ok !\n");
