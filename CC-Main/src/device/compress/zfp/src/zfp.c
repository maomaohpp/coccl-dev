...
zfp_type_size(zfp_type type)
{
  switch (type) {
    ...
    case zfp_type_bfloat16:
      return 2;
    case zfp_type_float16:
      return 2;
    case zfp_type_half:
      return 2;
    default:
...
zfp_stream_maximum_size(const zfp_stream* zfp, const zfp_field* field)
{
  ...
  // printf("ZFP_HEADER_MAX_BITS %d blocks * maxbits %lld stream_word_bits %d\n",ZFP_HEADER_MAX_BITS, blocks * maxbits, stream_word_bits);
...
zfp_compress(zfp_stream* zfp, const zfp_field* field)
{
  ...
  uint dims = zfp_field_dimensionality(field);
  uint type = (field->type == zfp_type_bfloat16 || field->type == zfp_type_float16 )? zfp_type_float : field->type;
