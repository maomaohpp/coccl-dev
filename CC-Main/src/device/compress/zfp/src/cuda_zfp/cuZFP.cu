...
//
template<typename T>
size_t encode(uint dims[3], int3 stride, int bits_per_block, T *d_data, Word *d_stream, zfpEXDType_t T1, cudaStream_t exec_stream)
{
  ...
  if(d == 1)
  {
    int dim = dims[0];
    int sx = stride.x;
    stream_size = cuZFP::encode1<T>(dim, sx, d_data, d_stream, bits_per_block, T1, exec_stream); 
  }
  else if(d == 2)
  {
...
template<typename T>
size_t decode(uint ndims[3], int3 stride, int bits_per_block, Word *stream, T *out, zfpEXDType_t T1, cudaStream_t exec_stream)
{
  ...
  else if(d == 1)
  {
    ...
    stream_bytes = cuZFP::decode1<T>(dim, sx, stream, out, bits_per_block, T1, exec_stream); 
...
void * offset_void(zfp_type type, void *ptr, long long int offset)
{
  ...
  if(type == zfp_type_float || type == zfp_type_bfloat16 || type == zfp_type_half)
  {
...
cuda_compress(zfp_stream *stream, const zfp_field *field)
{
  ...
  cudaStream_t exec_stream = *((cudaStream_t *)stream->exec.params);
  zfpEXDType_t exd_type = field->type == zfp_type_bfloat16 ? zfpBfloat16 :
                          (field->type == zfp_type_float16 || field->type == zfp_type_half) ? zfpFloat16 :
                          zfpOrigin;
  ...
  if(exec_stream == nullptr){
    ...
    // }
  if(field->type == zfp_type_float || field->type == zfp_type_bfloat16 || field->type == zfp_type_float16 || field->type == zfp_type_half)
  {
    float* data = (float*) d_data;
    stream_bytes = internal::encode<float>(dims, stride, (int)stream->maxbits, data, d_stream, exd_type, exec_stream);
  }
  else if(field->type == zfp_type_double)
  {
    double* data = (double*) d_data;
    stream_bytes = internal::encode<double>(dims, stride, (int)stream->maxbits, data, d_stream, exd_type, exec_stream);
  }
  else if(field->type == zfp_type_int32)
  {
    int * data = (int*) d_data;
    stream_bytes = internal::encode<int>(dims, stride, (int)stream->maxbits, data, d_stream, exd_type, exec_stream);
  }
  else if(field->type == zfp_type_int64)
  {
    long long int * data = (long long int*) d_data;
    stream_bytes = internal::encode<long long int>(dims, stride, (int)stream->maxbits, data, d_stream, exd_type, exec_stream);
  }
...
cuda_decompress(zfp_stream *stream, zfp_field *field)
{
  ...
  {
    float *data = (float*) d_data;
    decoded_bytes = internal::decode(dims, stride, (int)stream->maxbits, d_stream, data, exd_type, exec_stream);
    d_data = (void*) data;
  }
  else if(field->type == zfp_type_double)
  {
